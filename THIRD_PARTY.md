...
The [MIT license](LICENSE) covers VSpy Shark's own source, documentation and
...
- **libicsneo:** optional direct capture from Intrepid devices uses
  ...
  licenses. Disable this backend with `VSPYSHARK_WITH_ICSNEO=OFF` if unneeded.
- **Dear ImGui Test Engine:** the default development configuration enables it
  ...
  `VSPYSHARK_TEST_ENGINE=OFF` for a standalone build without that dependency.
...
`VSPYSHARK_SERVER_DIST` can copy a locally staged decoder bundle beside the
...
by their licenses. The MIT license on VSpy Shark does not relicense that bundle.
