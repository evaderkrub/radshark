...
`vspyshark.png` is the original artwork with transparent corners.
`vspyshark.ico` contains 16, 20, 24, 32, 40, 48, 64, 128 and 256 pixel frames.
Regenerate the ICO on Windows with `powershell -File scripts/build-icon.ps1`.

The Windows executable embeds resource 1 from `vspyshark.rc`. SDL3 adopts the
